...
def number_to_note_name(number):
    ...
    return NOTE_NAMES[int(round(number) % 12)]

def frequency_to_note_name(frequency, a4_freq):
    """ converts frequency to note name (for example: 440 returns 'A') """
    number = frequency_to_number(frequency, a4_freq)
    note_name = number_to_note_name(number)
    return note_name
...
class AudioAnalyzer(Thread):
    ...
    CHUNK_SIZE = 1024 // 4  # number of samples
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                max_freq = round(frequencies[np.argmax(magnitude_data)], 2)
                self.queue.put(max_freq)
                # if max_freq > 80:
                #     self.queue.put(frequency_to_note_name(max_freq, 440))
